...
export default function CreatorProfileView() {
  ...
  return (
    <div className="view">
      ...
      {/* Back */}
      <button className="btn-outline" style={{ marginBottom: 20 }} onClick={() => navigate({ to: '/feed' })}>
        ← Back to Vault
      </button>
      ...
      <div className="panel profile-hero">
        <div className="profile-video-thumb" style={{ background: creator.bg }}>
          ...
          <div className="play-btn" style={{ width: 52, height: 52, fontSize: 27 }}>▶</div>
          <div className="profile-video-label">Intro Video · 28s</div>
        </div>

        <div className="profile-hero-info">
          <div className="creator-name" style={{ fontSize: 33, marginBottom: 6 }}>{creator.name}</div>
          <div className="creator-handle" style={{ marginBottom: 8 }}>{creator.handle}</div>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10, marginTop: 10 }}>
            ...
            {creator.tags.filter(t => t !== 'Faith-Based').map(t => (
          ...
          <p style={{ fontSize: 19.5, color: 'var(--text-secondary)', lineHeight: 1.7, marginBottom: 20 }}>
            {creator.bio}
          </p>
          <div style={{ display: 'flex', gap: 8 }}>
            <button
              className="btn-gold"
              onClick={() => { navigate({ to: '/request' }); showToast(`Deal initiated with ${creator.name}`); }}
            >
              Request
            </button>
            <button className="btn-outline" onClick={() => showToast(`${creator.name} saved to vault`)}>
              ♡ Save
            </button>
          </div>
        </div>
      ...
      {/* Portfolio */}
      <div className="section-header" style={{ marginTop: 32, marginBottom: 16 }}>Portfolio</div>
      <div className="portfolio-grid">
        {creator.portfolio.map((emoji, i) => (
          <div className="portfolio-item" key={i} style={{ background: creator.bg }}>
            <span style={{ fontSize: 48 }}>{emoji}</span>
            <div className="play-btn" style={{ width: 32, height: 32, fontSize: 18 }}>▶</div>
          </div>
        ))}
      </div>

      {/* Past Brand Work */}
      <div className="section-header" style={{ marginTop: 32, marginBottom: 16 }}>Past Brand Work</div>
      <div className="panel">
        {creator.brands.map((brand, i) => (
          <div key={brand} className="brand-work-row" style={{ borderBottom: i < creator.brands.length - 1 ? '1px solid var(--border)' : 'none' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              <div className="brand-work-dot" />
              <div>
                <div style={{ fontFamily: 'var(--cinzel)', fontSize: 16.5, letterSpacing: 2, color: 'var(--gold-light)', textTransform: 'uppercase' }}>{brand}</div>
                <div style={{ fontSize: 16.5, color: 'var(--text-muted)', marginTop: 2 }}>UGC Campaign · Completed</div>
              </div>
            </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 32, marginBottom: 16 }}>
        <div>
          <div className="section-header" style={{ marginBottom: 4 }}>Reviews</div>
          <Stars rating={parseFloat(avgRating)} />
        </div>
      ...
      {showReviewForm && (
        <div className="panel" style={{ marginBottom: 20 }}>
          <div className="form-section-label" style={{ marginBottom: 14 }}>Your Review</div>
          <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
            {[1, 2, 3, 4, 5].map(i => (
              <button
                key={i}
                onClick={() => setReviewRating(i)}
                style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 24, color: i <= reviewRating ? 'var(--gold)' : 'var(--border)', padding: 0 }}
              >
                ★
              </button>
            ))}
