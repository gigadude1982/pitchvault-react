...
const CHATS = [
  {
    id: 0, name: 'Jordan Reeves', init: 'JR', campaign: 'Summer Launch · TikTok Video',
    status: 'In Review', statusClass: 'status-review', unread: true,
    revisions: 1, maxRevisions: 3,
    preview: 'Uploaded the final cut...',
    messages: [
      { from: 'creator', text: "Hey! I just uploaded the final cut for your review. Really happy with how it turned out.", time: '2:14 PM' },
      { from: 'brand', text: "Love the hook in the first 3 seconds. One ask: can you add a CTA overlay at the end?", time: '2:31 PM' },
      { from: 'creator', text: "Absolutely, I'll have that revision ready within the hour.", time: '2:33 PM' },
      { from: 'creator', file: 'final_cut_v2.mp4', fileType: 'MP4', delivery: false, time: '3:07 PM' },
    ],
  },
  {
    id: 1, name: 'Maya Chen', init: 'MC', campaign: 'Fall Collection · UGC Ad',
    status: 'Active', statusClass: 'status-active', unread: false,
    revisions: 0, maxRevisions: 3,
    preview: 'Can I adjust the deadline?',
    messages: [
      { from: 'brand', text: "Hi Maya! Excited for this one. Brief is attached.", time: 'Yesterday' },
      { from: 'creator', text: "Thank you! Can I adjust the deadline by 1 day? Want the lighting to be perfect.", time: 'Yesterday' },
      { from: 'brand', text: "No problem — take the time you need.", time: 'Yesterday' },
    ],
  },
  {
    id: 2, name: 'Caleb St. James', init: 'CS', campaign: 'Faith Community · Lifestyle',
    status: 'In Review', statusClass: 'status-review', unread: false,
    revisions: 2, maxRevisions: 3,
    preview: 'Revision 2 is ready',
    messages: [
      { from: 'creator', text: "Revision 2 is ready! Incorporated all your feedback from round one.", time: '10:15 AM' },
      { from: 'brand', text: "This looks excellent. Sending for final approval now.", time: '11:00 AM' },
      { from: 'creator', file: 'lifestyle_rev2.mp4', fileType: 'MP4', delivery: true, time: '11:04 AM' },
    ],
  },
  {
    id: 3, name: 'Sofia Voss', init: 'SV', campaign: 'TechDrop Launch · TikTok',
    status: 'Active', statusClass: 'status-active', unread: false,
    revisions: 0, maxRevisions: 3,
    preview: 'Excited to get started...',
    messages: [
      { from: 'brand', text: "Sofia, product sample is on its way. Let us know when it arrives!", time: 'Mon' },
    ...
    setChats(prev => prev.map(c =>
      c.id === activeChatId
        ? {
            ...
            messages: [...c.messages, { from: 'brand', file: file.name, fileType: ext, delivery: false, time: 'Now' }],
            ...
            <div className="revision-bar">
              ...
              <div className="revision-pips">
                {Array.from({ length: activeChat.maxRevisions }).map((_, i) => (
                  <div key={i} className={`revision-pip${i < activeChat.revisions ? ' used' : ''}`} />
                ))}
              </div>
              ...
              if (m.file) return (
                <div className={`msg from-${m.from}`} key={i}>
                  <div className="file-bubble">
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 18, fontWeight: 500, color: 'var(--text-primary)' }}>{m.file}</div>
